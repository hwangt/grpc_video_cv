...
class SteppableConv3dAs2d(nn.Conv2d):
    # __ignored_properties__ = ["temporal_footprint", "initialize_internal_state"]
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, **kwargs):
        ...
        # print(f'kernel_size {kernel_size.dtype}')
        # print(f'stride {stride.shape}')
        # print(f'dilation{dilation}')

        # TTH: TorchScript doesn't serialize methods yet, so add self.temporal_footprint attribute, set by renamed
        # method _temporal_footprint()
        # self.temporal_footprint = self._temporal_footprint()
        # self.temporal_footprint = None
        # print(self.__dict__)
    ...
    def initialize_internal_state(self, x):
        tf = self.temporal_footprint()
        self.internal_state = torch.cat(tf * [torch.zeros_like(x[0:1])])
